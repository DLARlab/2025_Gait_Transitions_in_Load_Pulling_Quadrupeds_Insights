...
classdef SLIP_Animation_Quad < OutputCLASS
    ...
    methods
        % Constructor:
        function obj = SLIP_Animation_Quad(P,plotPositions,FigOrAx,options)
        ...
        function obj = InitializePlots(obj, P)
            ...
            if string(obj.options.AnimationMode) == 'Detailed'
                obj.Title = text(obj.axes, x-1.3, 1.9, 'SLIP Model Animation','FontSize',15,'FontWeight','Bold');
                obj.PhaseDiagram = DrawPhaseDiagram(x,P, obj.axes);
            end

            axis(obj.axes, [-0.5 ,2.5,-0.1,2]);
        end
        ...
        function obj = update(obj, t, y, P)
            ...
            axis(obj.axes, [-1.5 + BodyJPos(1),1.5 + BodyJPos(1),-0.1,2]);
            drawnow();
        end
    end
end
...
function PhaseDiagram = DrawPhaseDiagram(x,P,ax)
    ...
    phase_box = patch(box_x, box_y,'white','LineWidth',1.5, 'Parent', ax);


    % Show the descriptions for the phase bars
    t_bl = text(pos_text(1,1),pos_text(1,2),'LH','FontSize',10, 'Parent', ax);
    t_fl = text(pos_text(2,1),pos_text(2,2),'LF','FontSize',10, 'Parent', ax);
    t_fr = text(pos_text(3,1),pos_text(3,2),'RF','FontSize',10, 'Parent', ax);
    t_br = text(pos_text(4,1),pos_text(4,2),'RH','FontSize',10, 'Parent', ax);
    ...
    if tEvents(1)<tEvents(2)
        phasebl_ = patch(pos_BL(1,:), pos_BL(2,:), 'white','EdgeColor','none', 'Parent', ax);
        phasebl  = patch(pos_BL(3,:), pos_BL(4,:), cp,'EdgeColor','none', 'Parent', ax);
    elseif tEvents(1)>tEvents(2)
        phasebl_ = patch(pos_BL(1,:), pos_BL(2,:), cp,'EdgeColor','none', 'Parent', ax);
        phasebl  = patch(pos_BL(3,:), pos_BL(4,:), 'white','EdgeColor','none', 'Parent', ax);
    end

    % Back right phase bar
    if tEvents(5)<tEvents(6)
        phasebr_ = patch(pos_BR(1,:), pos_BR(2,:),'white','EdgeColor','none', 'Parent', ax);
        phasebr  = patch(pos_BR(3,:), pos_BR(4,:), cp,'EdgeColor','none', 'Parent', ax);
    elseif tEvents(5)>tEvents(6)
        phasebr_ = patch(pos_BR(1,:), pos_BR(2,:), cp,'EdgeColor','none', 'Parent', ax);
        phasebr  = patch(pos_BR(3,:), pos_BR(4,:), 'white','EdgeColor','none', 'Parent', ax);
    end

    % Front left phase bar
    if tEvents(3)<tEvents(4)
        phasefl_ = patch(pos_FL(1,:), pos_FL(2,:),'white','EdgeColor','none', 'Parent', ax);
        phasefl  = patch(pos_FL(3,:), pos_FL(4,:), cp,'EdgeColor','none', 'Parent', ax);
    elseif tEvents(3)>tEvents(4)
        phasefl_ = patch(pos_FL(1,:), pos_FL(2,:), cp,'EdgeColor','none', 'Parent', ax);
        phasefl  = patch(pos_FL(3,:), pos_FL(4,:), 'white','EdgeColor','none', 'Parent', ax);
    end


    % Front right phase bar
    if tEvents(7)<tEvents(8)
        phasefr_ = patch(pos_FR(1,:), pos_FR(2,:),'white','EdgeColor','none', 'Parent', ax);
        phasefr  = patch(pos_FR(3,:), pos_FR(4,:), cp,'EdgeColor','none', 'Parent', ax);
    elseif tEvents(7)>tEvents(8)
        phasefr_ = patch(pos_FR(1,:), pos_FR(2,:), cp,'EdgeColor','none', 'Parent', ax);
        phasefr  = patch(pos_FR(3,:), pos_FR(4,:), 'white','EdgeColor','none', 'Parent', ax);
    end
